...
public:
    ...
    string *name = NULL; string *str = NULL;
...
bool IsVariant(VarObj *addr) //  check for corruption/validity, use <list> to track all open connections, avoid SEGFAULT
{
    ...
    bool b = false; list<VarObj>::iterator it = myVariants.begin();
    for (it = myVariants.begin(); it != myVariants.end(); ++it) 
        { if (it->addr == addr->addr)
        {b = true; break;}}
    if (!b) { ObjectErrStr = "Invalid Variant (unallocated memory or non-variant reference)"; ObjectErr = true; return false; }

    if (addr->canary_end == MAGIC) { ObjectErr = false; ObjectErrStr = "SUCCESS"; return true; }
...
void* ToVariant(U8ArrayHdl TypeStr, LStrHandle Data, LStrHandle Image, bool GetImage) {
    ...
    case TD::Void:    //  note, LabVIEW drops names on std flatten, though user can add
        V = &myVariants.emplace_back(VarObj(nm, true)); break;
    case TD::I32:
        V = &myVariants.emplace_back(VarObj(nm, *((int32_t*)  (**Data).str))); break;
    case TD::String:
        V = &myVariants.emplace_back(VarObj(nm, (char*) &((**Data).str[4]), *((int*) (**Data).str))); break;
    default:
        ObjectErr = true; ObjectErrStr = "Unsupported data type: " + (tStr->type);
        break;
    }
    if (V != NULL && GetImage) LV_str_cp(Image, string((char*) V, sizeof(VarObj)));
    return V;
}
...
//          the variant name plus the name itself.
    uInt8 NmSz = (V->name == NULL? 0: V->name->length() + 2 - V->name->length() % 2)
    ...
    case TD::String:
        ...
        if (V->name != NULL)
            {tStr->nm =  0x40; LvName[0] = V->name->length();
            memcpy(&LvName[1], V->name->c_str(), LvName[0]);}
    ...
    case TD::I8: case TD::U8:
        LV_str_cp(Data, string((char*) & V->data, sizeof(int8))); break;
    case TD::I16: case TD::U16:
        LV_str_cp(Data, string((char*) & V->data, sizeof(int16))); break;
    case TD::U32: case TD::I32:
        LV_str_cp(Data, string((char*) & V->data, sizeof(int32))); break;
    case TD::String:
        int sz; sz = 0; 
        if (V->str != NULL)
           {sz = V->str->length();
            LV_str_cp(Data, (char*) &sz, sizeof(int)); LV_str_cat(Data, *(V->str));}
